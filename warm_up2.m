function err = warm_up2(app, properties, rgb_cam, nir_cam, vlc)
...
while run == 1 && viewer_is_running == 1
    ...
    if properties.RGB_camera == 1
        frame_VIS = snapshot(rgb_cam); % get imgage from VIS camera
    end
    
    if properties.NIR_camera == 1
        frame_NIR = snapshot(nir_cam); % get imgage from VIS camera
    end
