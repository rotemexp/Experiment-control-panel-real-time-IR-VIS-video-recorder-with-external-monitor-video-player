...
if properties.warm_up == 1 && err == 0 % delay recording if needed
    err = warm_up2(app, properties, rgb_cam, nir_cam, vlc);
    tStart = tic;
end
    ...
    if properties.playVideofiles == 1
        ...
        if t(idx) - t(tLast_play) >= properties.playTime*1000 && playFlag == 1 % checks if elpased time is larger then X
            ...
            if properties.saveONblack == 1 && properties.save_data == 1  % save buffer data
                ...
                err = save_buffer2(app, properties, filename, playlist, buffer_VIS, buffer_NIR ,buffer_IR, video_idx-1, buff_idx); % update data to mat file
                buff_idx = 0;
            end
...
if properties.save_data == 1 && err ~= 1
    ...
    err = save_buffer2(app, properties, filename, playlist, buffer_VIS, buffer_NIR, buffer_IR, video_idx, buff_idx); % update data to mat file
